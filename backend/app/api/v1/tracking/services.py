...
# ===== Singletons pour services longs-vivants =====

def get_tracker() -> TrackerService:
  """Obtenir une instance singleton de TrackerService."""
  global _tracker_instance
  if _tracker_instance is None:
    settings = get_settings()
    tracker_config = {
      "lastfm": settings.secrets.get("lastfm", {}),
      "spotify": settings.secrets.get("spotify", {}),
      "euria": settings.secrets.get("euria", {}),
      "tracker": {
        "interval_seconds": settings.tracker_interval,
        "listen_start_hour": settings.listen_start_hour,
        "listen_end_hour": settings.listen_end_hour,
      },
    }
    _tracker_instance = TrackerService(tracker_config)
  return _tracker_instance


def get_scheduler() -> SchedulerService:
  """Obtenir une instance singleton de SchedulerService."""
  ...
  if _scheduler_instance is None:
    ...
    scheduler_config = {
      "euria": settings.secrets.get("euria", {}),
      "spotify": settings.secrets.get("spotify", {}),
      "scheduler": settings.app_config.get("scheduler", {}),
    }
    _scheduler_instance = SchedulerService(scheduler_config)
...
async def restore_active_services():
    ...
    try:
        ...
        for service_state in active_services:
            ...
            try:
                ...
                timeout = 10 if service_name == 'tracker' else 15
                ...
                if service_name == 'tracker':
                    ...
                    logger.info(f"ℹ️ Service obsolète ignoré: {service_name}")
                    save_service_state(service_name, False)
    ...
    logger.info(f"✅ Services restaurés: {restored_count}, échecs: {failed_count}")
    if not scheduler_found:
        logger.info("ℹ️ Scheduler non trouvé parmi les services actifs")
    return {
...
async def get_tracker_status():
    """Return in-memory tracker status for the frontend dashboard."""
    tracker = get_tracker()
    status = tracker.get_status()
    status["is_active"] = get_service_state("tracker")
    return status
...
async def get_all_services_status():
    """Aggregated status for tracker, scheduler and manual operations."""
    tracker = get_tracker()
    scheduler = get_scheduler()

    tracker_status = tracker.get_status()
    tracker_status["is_active"] = get_service_state("tracker")
