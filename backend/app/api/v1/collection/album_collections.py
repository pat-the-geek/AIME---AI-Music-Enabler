...
async def play_collection(
    ...
    """
    Play a collection on Roon audio system.
    
    Initiates playback of all albums in a collection on Roon (hi-fi audio system).
    Albums are played sequentially in order. Due to Roon API limitations, 
    individual song queuing is not possible - each album must complete before
    the next starts. Use Roon's native repeat-all function for continuous loop.
    
    **Path Parameters:**
    - `collection_id`: ID of collection to play (integer)
      - Type: integer
      - Required: Yes
      - Must exist and have albums
      - Example: 1, 42, 187
    
    **Request Body (PlayCollectionRequest):**
    ```json
    {
    ...
    **Request Parameters:**
    - `zone_name`: Target Roon zone/endpoint (string, optional)
      - Examples: "Living Room", "Bedroom", "Kitchen System"
      - If omitted: Uses currently active zone
      - Case-sensitive
      - Must be valid Roon zone
    
    **Response (200 OK - playback started):**
    ```json
    {
      "message": "Collection launched",
      "collection": "Summer Chill Sessions",
      "album_count": 24,
      "current_album": "Abbey Road",
      "current_artist": "The Beatles",
      "remaining_albums": 23,
      "note": "The album will play to completion. Use Roon controls to skip."
    }
    ```
    
    **Response Fields:**
    - `message`: Status message (string)
      - Always: "Collection launched"
    - `collection`: Collection name (string)
    - `album_count`: Total albums to play (int)
    - `current_album`: Now-playing album title (string)
    - `current_artist`: Now-playing artist (string)
    - `remaining_albums`: Albums queued after current (int)
    - `note`: User guidance on playback control
    
    **Roon Integration:**
    - Version: Roon API v2
    - Endpoint: Auto-discovery via mDNS
    - Protocol: WebSocket, bidirectional
    - Authentication: Token-based
    - Fallback: Uses last known zone
    
    **Playback Sequencing:**
    1. Verify collection exists
    2. Load all albums from collection
    3. Start first album on target zone
    4. Log queue for future reference
    5. Return current + queued info
    6. User: Manually advance (Roon UI)
    
    **Album Queue Limitation:**
    - Roon API: No queue manipulation
    - Workaround: Create multiple collections
    - Alternative: Use Roon bookmarks
    - Future: WebSocket queue if Roon updates API
    
    **Zone Selection:**
    - If zone_name omitted: Last active zone
    - If zone_name invalid: 400 Bad Request
    - If zone offline: 503 Service Unavailable
    - Multiple zones: Select explicitly
    ...
    **Error Scenarios:**
    - Collection not found: 404 Not Found
    - Collection empty: 400 Bad Request
    - Zone not found: 404 Not Found
    - Zone offline: 503 Service Unavailable or 502 Bad Gateway
    - Roon unavailable: 503 Service Unavailable
    - Playback already active: 200 OK (starts new collection)
    - Invalid collection_id: 422 Unprocessable Entity
    
    **Roon Availability:**
    - Requires Roon running on network
    - Requires Nucleus/Core or app
    - Requires valid zone endpoint
    - Network connectivity: Critical
    
    **Playback Control:**
    - Pause: Use Roon UI/remote
    - Skip: Use Roon controls
    - Stop: Pause, then select another source
    - Repeat: Roon settings (global)
    - Shuffle: Not available (collections are in order)
    
    **Performance:**
    - Start delay: 1-3 seconds
    - Album load: 50-200ms
    - Roon communication: 500ms-2s
    - Network dependency: Major factor
    
    **Frontend Integration:**
    ```javascript
    // Play collection on Roon
    async function playCollectionOnRoon(collectionId, zoneName = null) {
      try {
        const response = await fetch(
          `/api/v1/collections/${collectionId}/play`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              zone_name: zoneName  // Omit for last active zone
            })
          }
        );
        ...
        if (!response.ok) {
          if (response.status === 503) {
            showError('Roon not available');
          } else if (response.status === 404) {
            showError('Collection or zone not found');
          } else {
        ...
        console.log(`Playing: ${result.current_album} by ${result.current_artist}`);
        console.log(`${result.remaining_albums} more albums in queue`);
        
        updateNowPlaying(result.current_album, result.current_artist);
        return true;
      } catch (error) {
    ...
    // Play with zone selection
    await playCollectionOnRoon(1, 'Living Room');
    
    // Play on last active zone
    await playCollectionOnRoon(1);
    ```
    
    **Use Cases:**
    - Play curated collections
    ...
    **Advanced:**
    - Zone switching: Stop current, play on new zone
    - Sequential collections: Play after current finishes
    - Collection chaining: Bookmark collections for next play
    - Mood-based queuing: Create collection, play sequentially
    
    **Related Endpoints:**
    - GET /api/v1/collections/{id}/albums: Get collection contents
    - POST /api/v1/collections: Create collection
    - GET /api/v1/collections: List all collections
    """
    service = AlbumCollectionService(db)
